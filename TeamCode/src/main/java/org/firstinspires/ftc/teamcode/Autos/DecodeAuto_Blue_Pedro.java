...
//import com.pedropathing.geometry.Point; // Added back if needed, but we use Pose
...
@Autonomous(name = "DECODE Auto: Blue (Standard)", group = "Autonomous")
public class DecodeAuto_Blue_Pedro extends OpMode {
    ...
    // Coordinates
    private final Pose startPose = new Pose(62, 9.4, Math.toRadians(90));
    private final Pose shootPose1 = new Pose(56.79, 17.63, Math.toRadians(110));
    private final Pose intakeInterPose = new Pose(41.05, 35.46, Math.toRadians(110));
    private final Pose intakeDonePose = new Pose(17.27, 35.12, Math.toRadians(110));
    private final Pose shootPose2 = new Pose(56.69, 17.57, Math.toRadians(110));
    private final Pose parkPose = new Pose(56.36, 59.54, Math.toRadians(90));

    // Control Points (using Pose with heading 0 to satisfy type requirement)
    private final Pose intakeControlPoint = new Pose(50.37, 35.36, 0);
    private final Pose returnControlPoint = new Pose(49.04, 45.11, 0);
    ...
    private void startFiringSequence(int shots) {
        shotsToFire = shots; currentFireState = FireState.PUSH; fireTimer.resetTimer(); isFiring = true;
    }
    private void handleFiringLogic() {
        ...
        switch (currentFireState) {
            case PUSH:
                feeder.setPower(1.0);
                if (fireTimer.getElapsedTime() / 1e9 > FEED_TIME) { currentFireState = FireState.WAIT; fireTimer.resetTimer(); }
                break;
            case WAIT:
                feeder.setPower(0);
                if (fireTimer.getElapsedTime() / 1e9 > SHOT_DELAY) {
                    shotsToFire--;
                    if (shotsToFire > 0) currentFireState = FireState.PUSH;
                    else { isFiring = false; currentFireState = FireState.IDLE; }
